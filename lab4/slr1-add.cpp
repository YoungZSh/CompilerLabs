...
void make_table() {
    ...
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < collection[i].element.size(); j++) {
            ...
            if (k == str.length() - 1) {  // reduce
                if (item.left == start && item.right == start + string(1, CH)) {
                    action[i]['#'] = Content(2, item.id); // Accept
                } else {
                    for (set<char>::iterator it = follow[item.left].begin(); it != follow[item.left].end(); it++) {
                        action[i][*it] = Content(1, item.back);
                    }
                }
            } else {  // shift
                char a = str[k + 1];
                if (go[i][a] != -1) {
                    action[i][a] = Content(0, go[i][a]);
                }
            }
        }
    }
    ...
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < MAX; j++) {
            if (action[i][j].type == 0) {
                printf("ACTION[%d][%c] = shift %d\n", i, j, action[i][j].num);
            } else if (action[i][j].type == 1) {
                printf("ACTION[%d][%c] = reduce %d\n", i, j, action[i][j].num);
            } else if (action[i][j].type == 2) {
                printf("ACTION[%d][%c] = accept\n", i, j);
            }
        }
    }
    puts("********************GOTO TABLE*********************");
